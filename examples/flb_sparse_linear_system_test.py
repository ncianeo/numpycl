...
def A(x):
    ...
    pyclblast.gemv(npcl.queue, m, n, A_dev, x, res, a_ld=n, alpha=1.0, beta=0.0)
    return res

def AT(x):
    res = to_device(np.zeros_like(vecx))
    pyclblast.gemv(npcl.queue, n, m, AT_dev, x, res, a_ld=m, alpha=1.0, beta=0.0)
    return res

start = time()

x_flb, k = npcl.solvers.flb.solve_flb(A, AT, b_dev, stuck=1e-1, verbose=True, max_iter=50000)
